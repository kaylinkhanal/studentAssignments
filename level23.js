...
let highestMarksName = '';
for (let item of arr) {
  const [name, marks] = Object.entries(item)[0];
  if (marks > highestMarks) {
    highestMarks = marks;
    highestMarksName = name;
  }
...
//now using for in
let highestMarks1 = 0;
let highestMarksName1 = '';
for (let item in arr) {
  const [name, marks] = Object.entries(arr[item])[0];
  if (marks > highestMarks1) {
    highestMarks1 = marks;
    highestMarksName1 = name; 
  }
}
console.log(highestMarksName1);
