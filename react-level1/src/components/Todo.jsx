...
const Todo = () => {
  ...
  const addTodo = () => {
    if (input.trim() !== "" && assignee.trim() !== "" && deadline !== "") {
      setTodos([
        ...todos,
        { id: Date.now(), text: input, assignee: assignee, deadline: deadline, timeLeft: 60 * 60 * 24 },
      ])
      setInput("")
      setAssignee("")
      setDeadline("")
    ...
    (<div
      ...
      <div className="max-w-md w-full space-y-8">
        ...
        <div className="mt-8 space-y-6">
          ...
          {todos.length > 0 && (
            <ul
              className="bg-white shadow overflow-hidden rounded-md divide-y divide-gray-200">
              {todos.map((todo) => (
                <li
                  key={todo.id}
